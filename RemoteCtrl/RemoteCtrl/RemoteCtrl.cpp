...
void udp_server() {
	...
	std::list<sockaddr_in> lst_clients;
	...
	while (!_kbhit()) {
		...
		if (ret > 0) {
			if (lst_clients.size() <= 0) {
				lst_clients.push_back(client);
				printf("%s(%d):%s ip:%08X port:%d\r\n", __FILE__, __LINE__, __FUNCTION__, client.sin_addr.s_addr, ntohs(client.sin_port));
				ret = sendto(sock, (char*)buf.c_str(), ret, 0, (sockaddr*)&client, len);
				printf("%s(%d):%s\r\n", __FILE__, __LINE__, __FUNCTION__);
			}
			else {
				memcpy((void*)buf.c_str(), &lst_clients.front(), sizeof(lst_clients.front()));
				ret = sendto(sock, (char*)buf.c_str(), sizeof(lst_clients.front()), 0, (sockaddr*)&client, len);
				printf("%s(%d):%s\r\n", __FILE__, __LINE__, __FUNCTION__);
			}
		}
		else {
